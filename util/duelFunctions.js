...
function acceptDuel(interaction, victimPlayer, player){
    ...
    let ending;
        ...
        if(percent === 58) {
                ...
                if(player.hasGun()) {
                    ending = `${player.name} whips out his gun and absolutely blew ${victimPlayer.name} away, taking all of his coins and killing him!`
                }
                victimPlayer.kill();
            ...
            if(percent >= 24 && percent < 58) {
                ...
                if(player.hasGun()) {
                    ending = `${player.name} pulls out his gun extremely quickly, faster than ${victimPlayer.name} can react! And ${player.name} shoots them 2 times in the ribs before ${player.name}'s gun jams! ${victimPlayer.name} runs off in the sunset in search of medical treatment... **${player.name}'s gun decreased in durability**`
                    if(player.damageGun() === "Gun Broke") {
                        ending = `${player.name}, your gun broke!`
                    }
                } else {
                    ending = `${player.name} jumps at ${victimPlayer.name} before he had any chance to react! Severely beating him on the face giving him 14 bruises (dang)... until the cops stop the fight! And throw ${player.name} off of ${victimPlayer.name}!`
                }
                player.save();
                victimPlayer.save();
            }
            if(percent === 59) {
                ...
                if(victimPlayer.hasGun()) {
                    ending = `${victimPlayer.name} pulls out his fresh gun and absolutely pops ${victimPlayer.name}'s ass and steals all of his coins, killing him the process!`
                } else {
            ...
            if(percent > 59 && percent <= 100){
                if(victimPlayer.hasGun()){
                    ending = `${victimPlayer.name} pulls out his gun extremely quickly, faster than ${player.name} can react! And ${victimPlayer.name} shoots them 2 times in the ribs before ${victimPlayer.name}'s gun jams! ${player.name} runs off in the sunset in search of medical treatment... **${victimPlayer.name}'s gun decreased in durability**`
